...
-module(uurl_db).
-author('gleicon <gleicon@gmail.com>').
-export([create/3, update/4, stats/1, check_url/1, translate_url/1]).

-define (UNIQUE_ID_REF, "URLSHORT:UNIQ").
-define (OBJECT_ID_REF, "OBJID:").
-define(AS_JSON(K, V), bson_to_json(proplists:get_value(K, V))).

create(URL, Referer, Visitor) ->
	Id = create_unique_id(),
	UID = base62:encode(Id),
	emongo:insert(uurl_pool, "urls", [{"uuid", Id}, {"uurl", UID}, {"url", URL}, {"clicks", 0}, {"referers", [Referer]}, 
		{"visitors", [Visitor]}, {"clicks_per_minute", []}, {"date", calendar:now_to_datetime(now())}]),
	cache_url(URL, UID),
	{ok, UID}.

update(UID, URL, Referer, Visitor) ->
	Id = base62:decode(UID),
	emongo:update(uurl_pool, "urls", [{"uuid", Id}], [
										{"$inc", [{"clicks", 1}]},
										{"$inc", [{["clicks_per_minutea", get_current_date()], 1} ]},
...
stats(UID) -> 
	...
	Obj = {struct, [
	    ...
	    {cpm, ?AS_JSON(<<"clicks_per_minutea">>, Stats)},
...
get_current_date() ->
		...
		lists:flatten(DP++lists:sublist(TP, 1, 2)).
